...
$TimeSub180 = (Get-Date -DisplayHint Time).AddMinutes(-180)
  
    # ALO 8 - insertado 27-1-2024
    Write-Host "***********************************8***************************************"
    write-host "ALO - m s abajo aparece la HORA del inicio de chequeo de log de FIREWALL"
    write-host " "
    $TimeSub180
    Write-Host "*************************************************************************"
    # ALO  8 - insertado 27-1-2024
...
If ($Time -LIKE "00:00:**") {
    # It's midnight
    Write-Host "************* ALO - Es Medianoche. Se borro regla de firewall **************************"
    Set-NetFirewallRule -DisplayName "RDC Brute Force Prevention" -RemoteAddress '255.255.255.254'
} else {
    # Do nothing
     Write-Host "************* ALO - NO es Medianoche. Se mantiene regla de firewall **************************"
}
...
$CSVFile | Select -Skip 5 | Where {$_.Date -EQ $Date -and $_.Time -GE $TimeSub180.ToString("HH:mm:ss") -and $_.SourceIP -NE '127.0.0.1' -and $_.DestPort -EQ '3389'} | Group-Object -Property SourceIP -NoElement | Where {$_.Count -GT '10'} | Sort-Object -Descending | % {
